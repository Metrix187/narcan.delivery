...
// ---------------------------------------------------------------------------
// Service worker, emitted by the build so the cache version and precache list
// always match this deploy. Deliberately conservative:
//   - navigations: network first (short timeout), cache fallback, cached '/'
//     as the offline landing (it has the four steps and the picker)
//   - same-origin static assets: cache first (js urls are content-versioned)
//   - cross-origin requests (analytics beacon): untouched
// A stale-forever bug is the worst failure mode a SW can have, so the cache
// name embeds the content hash and old caches are purged on activate.
// ---------------------------------------------------------------------------
async function emitServiceWorker(ver) {
  const precache = [
    '/', '/card/', '/es/', '/404.html', '/manifest.json', '/sw-register.js',
    `/app.js?v=${ver['/app.js']}`,
    `/data.js?v=${ver['/data.js']}`,
    `/card/card.js?v=${ver['/card/card.js']}`,
    '/icon-192.png', '/icon-512.png', '/apple-touch-icon.png',
    '/fonts/InterVariable.woff2', '/fonts/InterVariable-Italic.woff2',
    '/fonts/Fraunces-Variable-latin.woff2', '/fonts/Fraunces-Variable-latin-ext.woff2',
  ];
  try { await fs.access(path.join(ROOT, 'es', 'card', 'index.html')); precache.push('/es/card/'); } catch { /* not built yet */ }
  const version = createHash('sha256').update(JSON.stringify([ver, precache])).digest('hex').slice(0, 10);
  const sw = `/* generated by build.mjs — edit the template there, not this file */
'use strict';
const CACHE = 'narcan-${version}';
...
`;
  ...
  console.log(`✓ Wrote sw.js (cache narcan-${version}, ${precache.length} precached)`);
}
